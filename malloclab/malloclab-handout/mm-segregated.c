...
/*
 * free_list[0]     list of block, block size <= 16
 * free_list[1]     list of block, block size <= 32
 * free_list[2]     list of block, block size <= 64
 * free_list[3]     list of block, block size <= 128
 * free_list[4]     list of block, block size <= 256
 * free_list[5]     list of block, block size <= 512
 * free_list[6]     list of block, block size <= 1024
 * free_list[7]     list of block, block size <= 2048
 * free_list[8]     list of block, block size <= 4096
 * free_list[9]     list of block, block size > 4096
 */
#define FREE_LIST_OFF_FIRST     0
#define FREE_LIST_OFF_LAST      9
static list_node_t **free_list = NULL;
...
/*
 * 根据 size 大小计算 free_list 的偏移量
 */
static int list_size_off (size_t size) 
{
    int off = 0;
    ...
    if (off < FREE_LIST_OFF_FIRST) {
        off = FREE_LIST_OFF_FIRST;
    } else if (off > FREE_LIST_OFF_LAST) {
        off = FREE_LIST_OFF_LAST;
    }
...
static void list_remove_node (void *node)
{
    ...
    size_t       size = GET_SIZE(HDRP(node));
    int          free_list_off;

    free_list_off = list_size_off(size);
    ...
    if (prev) {                 /* 移除非首个节点 */
        ...
        if (next) {
            free_list[free_list_off] = next;
            free_list[free_list_off]->prev = NULL;
        } else {
            free_list[free_list_off] = NULL;
        }
    }
}
...
static void list_insert_node (void *node)
{
    ...
    if (free_list[free_list_off]) {
        free_list[free_list_off]->prev = node;
    }
    ((list_node_t *)node)->prev = NULL;
    ((list_node_t *)node)->next = free_list[free_list_off];
    free_list[free_list_off] = node;
}
...
static void *find_fit (size_t size)
{
    ...
    for (free_list_off = list_size_off(size); free_list_off <= FREE_LIST_OFF_LAST; free_list_off++) {
        for (node = free_list[free_list_off]; node != NULL; node = node->next) {
...
int mm_init(void)
{
    ...
    if ((free_list = mem_sbrk(16 * sizeof(list_node_t *))) == (void *)-1) {
        return -1;
    }
    ...
    heap_listp += (2 * WSIZE);
    memset(free_list, 0, 16 * sizeof(list_node_t *));
